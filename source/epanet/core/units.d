...
class Units
{
  public:
    ...
    mixin template QuOptAliases()
    {
        static foreach(name; __traits(allMembers, Quantity))
            mixin("alias " ~ name ~ " = " ~ " __traits(getMember, Quantity, name);");
    }
    mixin QuOptAliases;
    ...
    void setUnits(ref Options options){
    ...
    double factor(Units.Quantity quantity) {return factors[quantity];}

    string name(Units.Quantity quantity) {return names[quantity];}

  private:

    double[MAX_QUANTITIES]      factors;  //!< array of unit conversion factors
    string[MAX_QUANTITIES]      names;    //!< array of unit names
    ...
    void   setPressureFactor(const int unitSystem, ref Options options){
