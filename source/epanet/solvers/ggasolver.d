 ...
 *
 */
module epanet.solvers.ggasolver;
...
static const double Huge = double.max;

// step sizing enumeration
enum {FULL, RELAXATION, LINESEARCH}
alias StepSizing = int;
//! \class GGASolver
//! \brief A hydraulic solver based on Todini's Global Gradient Algorithm.

class GGASolver : HydSolver
{
  public:
    ...
    this(Network nw, MatrixSolver ms){
        ...
        dH.length = nodeCount; dH[] = 0; // nodal head changes
        dQ.length = linkCount; dQ[] = 0; // link flow changes
        xQ.length = nodeCount; xQ[] = 0;           // nodal excess flow (inflow - outflow)

        hLossEvalCount  = 0;
        trialsLimit     = 0;
        reportTrials    = cast(bool)network.option(Options.REPORT_TRIALS);
    ...
    override int solve(double tstep_, ref int trials){
        ...
        while ( trials <= trialsLimit )
        {
            ...
            if ( errorCode >= 0 )
            {
                ...
                network.msgLog.writef("\n %s%s", s_IllConditioned, node.name);
  ...
  private:
    ...
    Array!double dH;       // head change at each node (ft)
    Array!double dQ;       // flow change in each link (cfs)
    Array!double xQ;       // node flow imbalances (cfs)
    ...
    void setFixedGradeNodes(){
        ...
        {
            foreach (Node tankNode; network.nodes)
            {
    ...
    void setNodeCoeffs(){
        for (int i = 0; i < nodeCount; i++)
        {
            ...
            if ( !node.fixedGrade )
            {
                ...
                matrixSolver.addToRhs(i, cast(double)xQ[i]);
            }
    ...
    void setValveCoeffs(){
        foreach (Link link; network.links)
        {
            ...
            if ( link.isPRV() )
            {
                matrixSolver.addToRhs(n1, cast(double)xQ[n2]);
            }
            ...
            if ( link.isPSV() )
            {
                matrixSolver.addToRhs(n2, cast(double)xQ[n1]);
            }
        }
    }
    ...
    double findErrorNorm(double lamda){
        hLossEvalCount++;
        return hydBalance.evaluate(lamda, cast(double*)&dH[0], cast(double*)&dQ[0],
                                        cast(double*)&xQ[0], network);
    }
    ...
    bool linksChangedStatus(){
        ...
        foreach (Link link; network.links)
        {
            ...
            if (oldStatus != link.status)
            {
                if ( reportTrials )
                {
                    network.msgLog.writef("\n%s", link.writeStatusChange(oldStatus));
                }
                result = true;
            }
        }
    ...
    void reportTrial(int trials, double lamda){
        network.msgLog.writef("\n\n%s%d:",  s_Trial, trials);
        network.msgLog.writef("\n%s%f", s_StepSize, lamda);
        network.msgLog.writef("\n%s%f", s_TotalError, errorNorm);
        ...
        {
            network.msgLog.writef("%s%s", s_ForLink, network.link(hydBalance.maxHeadErrLink).name);
        }
        ...
        {
            network.msgLog.writef("%s%s", s_AtNode, network.node(hydBalance.maxFlowErrNode).name);
        }

        // ... report link with largest change in flow rate

        network.msgLog.writef("\n%s%f %s", s_FlowChange, hydBalance.maxFlowChange *
            network.ucf(Units.FLOW), network.getUnits(Units.FLOW));
        if ( hydBalance.maxFlowChangeLink >= 0 )
        {
            network.msgLog.writef("%s%s", s_ForLink, network.link(hydBalance.maxFlowChangeLink).name);
        }

        // ... report total link flow change relative to total link flow

        network.msgLog.writef("\n%s%f", s_TotFlowChange, hydBalance.totalFlowChange);
    }
}